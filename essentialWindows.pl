...
#../Tn_SeqAnalysisScripts/essentials.pl --wig essentialTest/2Gpval.wig --excel essentialTest/2GTAsitesInsertions.xls --ref=NC_003028b2.gbk --cutoff 15 --essential tigr4_genome.fasta --csv essentialTest/2Gessential.csv --essentialWig essentialTest/2GunmatchedInsertions.wig --step 10 --size 500 results/L1_2394eVI_PennG.csv results/L3_2394eVI_PennG.csv results/L4_2394eVI_PennG.csv results/L5_2394eVI_PennG.csv results/L6_2394eVI_PennG.csv
...
while (($result != -1) and ($pos!=scalar @insertPos)) { #while loop continues until reach end of fasta genome string (-1) or run out of insertion sites
    my $yes=0;
...
while (($result != -1) and ($pos!=scalar @insertPos)) {
    ...
    my @sites=($result,$yes);
    push (@selector,$yes);    #push the 0 or 1 onto the array @selector---going to use this to draw random sets for the null distribution
    push (@allTAsites,\@sites); #array of all TA sites and whether insertion happended
    #print $result, " ", $yes,"\n";
    $offset = $result + 1;
    $result = index($genome, 'TA', $offset);
    $countTA++;
